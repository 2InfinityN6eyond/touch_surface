...
model = torch.jit.load('hand.pts', map_location=torch.device(target_device))
print('target... ', target)
...
def transform_net_input(tensor, source_img, hand_rect=None, tensor_idx=0):
    ...
    tensor[tensor_idx][0] = torch.tensor(r, device=torch.device(target_device)).float()
    tensor[tensor_idx][1] = torch.tensor(g, device=torch.device(target_device)).float()
    tensor[tensor_idx][2] = torch.tensor(b, device=torch.device(target_device)).float()
    return ratio

def detect_bbox(input_image):
    tensor = torch.zeros([1, 3, TRAIN_IMAGE_HEIGHT, TRAIN_IMAGE_WIDTH], device=torch.device(target_device))
...
def detect_hand(input_image, hand_rect):
    ...
    tensor = torch.zeros([len(hand_rect), 3, TRAIN_IMAGE_HEIGHT, TRAIN_IMAGE_WIDTH], device=torch.device(target_device))
