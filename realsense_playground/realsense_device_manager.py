...
class DeviceManager:
    ...
    def get_device_intrinsics(self, frames):
        ...
        keys  : serial : Serial number of the device
        values: [key] : Intrinsics of the corresponding device
        """
        device_intrinsics = {}
...
if __name__ == "__main__":
    try:
        ...
        pp.pprint(device_manager._enabled_devices.keys())        
        ...
        while True :
            ...
            '''
            for k, frame_set in frames.items() :
                print(k)
                for key, frame in frame_set.items() :
                    print(f"{type(key)}  {key}:{type(frame)}", end = " ")
                print()
                #print(type(frame[""]))
